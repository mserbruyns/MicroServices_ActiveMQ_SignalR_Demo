...
namespace Pencil42.PakjesDienst.Web
{
        ...
        : BackgroundService
    {
        ...
        public override async Task StartAsync(CancellationToken cancellationToken)
       {
            ...
            //multiple
            Message message1;
            while ((message1 = await _receiverLink.ReceiveAsync()) != null)
            {
                //_receiverLink.(message1);
                Console.WriteLine(message1.Body.ToString());
                var pakjeMessage = message1.GetBody<PakjeMessage>();
        ...
        private async void OnMessage(IReceiverLink receiverLink, Message message)
        {
            _logger.LogDebug($"QueueListener Message received");

            var pakjeMessage = message.GetBody<PakjeMessage>();
            //_receiverLink.Accept(message);
            _hubcontext.Clients.All.SendAsync("broadcastMessage", "queue", pakjeMessage);
            //Task.Run(() => _hubcontext.Clients.All.SendAsync("broadcastMessage", "queue", pakjeMessage));
        }
